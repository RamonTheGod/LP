<?php
	...
	if (!$_GET['u']) {
		...
		$gu = str_replace(' ','',$_GET['u']);
		$sql = "select user_id,username from users where user_id = '$gu'";
		// $result = @mysql_db_query('ramonlp_ramonlp', $sql);
		$result = @mysqli_query($conn, $sql);
		
		// while ($r = mysql_fetch_array($result)) {
		while ($r = mysqli_fetch_assoc($result)) {
			$ousername = $r['username'];
			$ouserid = $r['user_id'];
		}
		
		$sql = "select * from styles where user_id = $ouserid";
		// $result = @mysql_db_query('ramonlp_ramonlp', $sql);
		$result = @mysqli_query($conn, $sql);
...
<?php
	$slotcounter = 0;
	$rowcounter = 0;
	$sql = "select c.category,l.linkurl,l.linktitle,l.user_id from categories c left join links l on (c.cat_id = l.cat_id) where c.user_id = $ouserid and l.private = 0 and l.adult = 0 order by c.place,c.category, l.linktitle, l.linkurl";
	// $result = @mysql_db_query('ramonlp_ramonlp', $sql);
	$result = @mysqli_query($conn, $sql);
...
<script type="text/javascript" src="http://pagead2.googlesyndication.com/pagead/show_ads.js">
		...
		if ($r['user_id'] == $ouserid) {
			echo '<a href="' . stripslashes(urldecode($r['linkurl'])) . '">' . stripslashes(urldecode($r['linktitle'])) . "</a><br/>";
		}
	}
